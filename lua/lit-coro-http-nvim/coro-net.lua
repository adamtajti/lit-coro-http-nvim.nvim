...
local uv = vim.uv
local wrapStream = require("lit-coro-http-nvim.coro-channel").wrapStream
local wrapper = require("lit-coro-http-nvim.coro-wrapper")
...
local function connect(options)
	...
	local dsocket
	if options.tls then
		if not secureSocket then
			secureSocket = require("lit-coro-http-nvim.secure-socket")
		end
