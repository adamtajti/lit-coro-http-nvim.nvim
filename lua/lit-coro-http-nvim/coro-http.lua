-- Modifications:
-- Requires lit-coro-http-nvim local modules to use vim.uv
...
local httpCodec = require("lit-coro-http-nvim.http-codec")
local net = require("lit-coro-http-nvim.coro-net")
...
local function saveConnection(connection)
	if connection.socket:is_closing() then
		return
	end
